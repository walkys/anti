...
* 본 견적은 실시간 자동 산출 가견적입니다.`;
    ...
    // --- 리뉴얼 메인 납품사례 정적 프리셋 버튼 연동 ---
    const presetButtons = document.querySelectorAll('.btn-calc-preset');
    presetButtons.forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            // 1. 상품 프리셋 선택
            const prodId = btn.getAttribute('data-preset-prod');
            const targetProd = PRODUCT_DATABASE.find(p => p.id === prodId);
            if (targetProd) {
                selectProduct(targetProd);
            }
            
            // 2. 수량 프리셋 대입
            const qty = btn.getAttribute('data-preset-qty');
            if (qty && qtyNumber) {
                qtyNumber.value = qty;
                if (qtySlider) qtySlider.value = qty;
                if (typeof updateSliderMarkers === 'function') {
                    updateSliderMarkers(parseInt(qty));
                }
            }
            ...
            const printZones = ['front', 'back', 'sleeve'];
            printZones.forEach(zone => {
                const methodAttr = btn.getAttribute(`data-preset-print-${zone}`);
                const el = zoneElements[zone];
                if (!el) return;
    ...
    // --- 리뉴얼 메인 및 GNB 견적기 트리거 버튼 연동 ---
    const openQuoteModalTriggerIds = ['btn-order', 'hero-btn-order', 'process-cta-btn', 'cta-order-btn'];
    openQuoteModalTriggerIds.forEach(id => {
